...
SaveFitStats2<-function(res, res0, i){

  measType<-MEASCAT[i]
  
  est<-round(res$b['Xm',], digits=2)
  ci.lb<-round(res$ci.lb[2], digits=3)
  ci.ub<-round(res$ci.ub[2], digits=3)
  includes1<-sum(ci.lb < 1 & ci.ub > 1) #if True (1), then ci includes 1
  studies<-res$k
  ...
  result<-data.frame(measType, studies, pR2, est, ci.lb, ci.ub, includes1)
