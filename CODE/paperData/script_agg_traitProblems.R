#paperData_aggregate/script_agg_traitProblems.R
#Manually aggregate problematic aggIDs


#Decide how to deal with these aggIDs
agg.Probs<-c(agg.Probs,agg.unitProbs)
agg.Probs
traitAgg[traitAgg$aggID %in% agg.Probs,]




### obsID == 29.01 and 29.02 ####################################################
probAggIDs<-agg.Probs[grepl('29', agg.Probs)]
probAggIDs
...
### obsID == 236.01 ####################################################
probAggIDs<-agg.Probs[grepl('236.01', agg.Probs)]
probAggIDs
...
### obsID == 716.01 ####################################################
probAggIDs<-agg.Probs[grepl('716.01', agg.Probs)]
probAggIDs
...
### obsID == 353.01 ####################################################
probAggIDs<-agg.Probs[grepl('353.01', agg.Probs)]
probAggIDs
...
# Identify the Aggregation fxn based on this particular dataset - Make a custom 'Aggregate' fxn
# Notes say that there should be a weighted average to aggregate values across plant parts (leaflets, rachis, reproductive), with n as representative of relative abundance.  
# This is already how CalcMean works, so just change operation1 to say 'average'
dfprob[,'operation1']<-'average'
...
agg.result.list<-list()
for (i in 1:length(AGGID)){
  sub.Agg<-dfprob[dfprob$aggID == AGGID[i],]
...
# Put the aggregated data back into the original data sheet
final<-aggTab.prob
df.new<-traits.new3 # has already been updated with the non-problematic traitAgg data
colvartype<-'traitVarType'
colfinals<-c('mean','var','n')
coldfs<-c('traitMean','traitVar_VAR','traitN')
invnat<-FALSE
# Insert new values and update variance type
traits.new4<-InsertAggs(final=final, df.new=df.new, colvartype=colvartype, colfinals=colfinals, coldfs=coldfs, invnat=invnat)
#View(traits.new4)
