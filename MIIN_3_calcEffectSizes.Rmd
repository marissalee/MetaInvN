...
dat.obs1<-dat.obs[,c('paperID','obsID','measCat',
                 'n1i', 'm1i', 'sd1i', 'n2i', 'm2i', 'sd2i',
                 'measQuality',
                 'ecosystCat','studyType','InvNfix','NatNfix','invGenera')]
...
pHist_ES<-ggplot(dat1.meas, aes(x=yi)) + facet_wrap(~measCat, scales='free', ncol=3) + geom_histogram() +
...
paste('Removed outlier effect size values in nh, no, ph, soiln.')
paste('Effect size values look normally-distributed - except cn, littercn, litterpercN, probably because there was not very much data')
...
paste('Most unit-std soil meas. values look normally-distributed after log-transforming some measurement types - except cn, litterbiom, littercn, litterpercN, probably because there was not very much data')
```
...
write.table(papers.c, file=paste(synthdataPath,newfilename, sep='/'), sep='\t')
